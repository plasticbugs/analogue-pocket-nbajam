#!/bin/sh
# Compile the NBA Jam Pocket core with Quartus 18.1 in Docker (x86 emulation on ARM Macs).
#
...
FLOW=${1:-compile}
case "$FLOW" in
    map) CMD="quartus_map --read_settings_files=on projects/nbajam_pocket.qpf -c nbajam_pocket" ;;
    compile) CMD="quartus_sh --flow compile projects/nbajam_pocket.qpf" ;;
