...
(
    ...
    input   E,
    input   Q,
    // Synchronous operation. Upstream clocks this core on the falling edges of
    // E and Q, which on an FPGA means clocking it from decoded counter logic:
    // routed on general fabric, glitch-prone, and invisible to timing analysis
    // (Quartus reports "determined to be a clock but found without an
    // associated clock assignment"). Deterministic in simulation, unreliable in
    // silicon - the Arch Rivals sound board was silent on hardware while
    // matching MAME exactly in Verilator. clk is the real clock; cen_e and
    // cen_q pulse for one clk where those falling edges would have been.
    input   clk,
    input   cen_e,
    input   cen_q,
    output  BS,
...
// LOCAL MODIFICATION (see README.md): power up with no NMI pending.
// NMILatched is only ever written by an asynchronous block, so with no initial
// value it powers up as 0 = "NMI pending" and the CPU takes a spurious NMI on
// the first instruction after reset. The reset path cannot clear it: NMIClear
// is held at 0 during reset, and the NMI mask is released on the first
// non-reset cycle because reset itself changes S.
reg     NMILatched = 1'b1;
reg     NMISample;
reg     NMISample2;
reg     NMIClear;
...
// Was clocked off NMISample2 falling with an asynchronous set - a register
// output used as a clock. Same behaviour, detected synchronously instead.
reg NMISample2_q = 1'b0;   // must match NMISample2's power-up value,
                          // or the first clock looks like a falling edge and
                          // latches an NMI the CPU never received
always @(posedge clk)
begin
    NMISample2_q <= NMISample2;
    if (wNMIClear == 1)
        NMILatched <= 1;
    else if (NMISample2_q && !NMISample2)      // the falling edge
        NMILatched <= (NMIMask == 0) ? 1'b0 : 1'b1;
end
        ...
        begin
            ...
            case (AddrModeType)
                ...
                begin
                    ...
                    begin
                        a_nxt = {8{b[7]}};
                        // LOCAL MODIFICATION (see README.md): SEX sets the
                        // condition codes from the 16-bit result, which
                        // upstream omits. Per the MC6809 datasheet: N from the
                        // result's sign, Z if the result is zero, V cleared.
                        cc_nxt[CC_N_BIT] = b[7];
