#!/usr/bin/env python3
"""Build the Pocket .rom image for the NBA Jam core from a MAME romset.
...
MAME `nbajam` romset -- either the zip or a directory of loose files -- checks
...
Supported MRA elements (the standard MiSTer subset):
  ...
  <interleave output="16"> <part name=.. crc=.. map="01"/> ... </interleave>
        ...
        This builder read maps left to right until 2026-09-25, and this core's
        .mra files were written that way, so the standard `mra` tool built a
        different image from them (it only warns on an md5 mismatch).  Both
        now agree, and the md5 in each .mra is checked by both.

Nothing but Python 3 is required. The same .mra works with the standard MiSTer
mra tools (mra-tools-c), which build a byte-identical image.
