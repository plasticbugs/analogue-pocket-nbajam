...
Layout of `nbajam.rom` byte by byte, the SDRAM word address of each region,
and the same constants in `target/pocket/nbajam_mem.sv` and `sim/tb_mem.cpp`.
...
Every multicycle in `projects/nbajam_pocket.sdc`, why everything its filter
